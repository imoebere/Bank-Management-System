    ...
    <div class="row  g-lg-5 py-5">
      ...
      <?php 
                  if (isset($_POST['Submit'])){
                    ...
                    $Amount_Deposit = trim($_POST['Amount_Deposit']);
                    $select = "SELECT Acc_No, Acc_Type, Balance FROM Account_tbl where Acc_No ='$acc_num'";
                    $query=mysqli_query($conn,$select);
                    $row_num = mysqli_num_rows($query);
                    while($row=mysqli_fetch_assoc($query)){
                        $Acc_No = $row['Acc_No'];
                    ...
                    if ($row_num == 0){
                        echo '<script type="text/javascript">alert("Invalid Account Number!")</script>'.'<br>';
                        echo '<script type="text/javascript">alert("See the Customer Care!")</script>'.'<br>';
                    }else{
                        $total = ((int)$Balance + (int)$Amount_Deposit);
                        $insert = "INSERT INTO Account_tbl (Acc_No,Acc_Type,Deposit,Balance)
                        VALUES('$acc_num','$Acc_Type','$Amount_Deposit','$total')";
                        $result=mysqli_query($conn,$insert);
                        if ($insert){
                            echo "<strong>You have successfully Deposit the Sum of: {$Amount_Deposit} Into {$acc_num}</strong>";
                            echo "<h3>Thanks for Banking With Us</h3>";
                        }

                        /*$select = "SELECT Balance FROM Account_tbl where Acc_No ='$acc_num'";
                        $query=mysqli_query($conn,$select);
                        $row_num = mysqli_num_rows($query);
                        while($row=mysqli_fetch_assoc($query)){
                            $Balance = $row['Balance'];
                        ...
                        $insert2 = "INSERT INTO Account_tbl (Acc_No, Balance)
                        VALUES('$acc_num','$total')";
            ...
            <h3>ADD NEW ACCOUNT</h3>
                <div class="input-group mb-3">
                    ...
                    <input type="number" class="form-control"  required name="acc_num" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>AMOUNT TO DEPOSIT:</strong></span>
                    <input type="number" class="form-control" required name="Amount_Deposit" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                <div class="input-group mb-3">
