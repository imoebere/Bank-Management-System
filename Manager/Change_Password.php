      ...
      <div class="col-md-10 col-lg-6">
            <form method="post" enctype="multipart/form-data" class="p-4  border rounded-3 bg-light">
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="password" class="form-control" autocomplete="off" required name="old_pwd" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>NEW PASSWORD:</strong></span>
                    <input type="password" class="form-control" autocomplete="off" required name="new_pwd" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>

                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>CONFIRM PASSWORD:</strong></span>
                    <input type="password" class="form-control" autocomplete="off"  required name="con_pwd" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                ...
                <?php 
                  if (isset($_POST['Submit'])){
                    ...
                    $select = "SELECT Customer_ID, pwd FROM Customer_tbl where pwd ='$old_pwd' and Customer_ID='$customer_id'";
                    $query=mysqli_query($conn,$select);
                    $row_num = mysqli_num_rows($query);
                   
                    if ($row_num==0){
                        echo '<script type="text/javascript">alert("Wrong Password! Or Wrong Customer ID")</script>'.'<br>';
                    }else{
                    
                    if($new_pwd == $con_pwd){
                        $update = "UPDATE Customer_tbl SET pwd='$new_pwd' WHERE Customer_ID='$customer_id'";
