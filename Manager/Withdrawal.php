    ...
    <div class="row  g-lg-5 py-5">
      ...
      <?php 
                  if (isset($_POST['Submit'])){
                    ...
                    $Amount_Withdraw = trim($_POST['Amount_Withdraw']);
                    $select = "SELECT * FROM Account_tbl where Acc_No ='$acc_num'";
                    $query=mysqli_query($conn,$select);
                    $row_num = mysqli_num_rows($query);
                    while ($row=mysqli_fetch_assoc($query)){
                        $Balance = $row ['Balance'];
                        $Acc_Type = $row ['Acc_Type'];
                            ...
                            $total = ((int)$Balance - (int)$Amount_Withdraw);
                            if($total >= 0){
                            $insert = "INSERT INTO Account_tbl (Acc_No,Acc_Type,Withdrawal,Balance)
                            VALUES('$acc_num','$Acc_Type','$Amount_Withdraw','$total')";
                            $result=mysqli_query($conn,$insert);
                            echo "<strong>You have successfully Withdraw the Sum of: {$Amount_Withdraw} From {$acc_num}</strong>";
            ...
            <h3>ADD NEW ACCOUNT</h3>
                ...
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>AMOUNT TO WITHDRAW:</strong></span>
                    <input type="number" class="form-control" required name="Amount_Withdraw" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                <div class="input-group mb-3">
