    ...
    <div class="row  g-lg-5 py-5">
      ...
      <?php 
                  if (isset($_POST['Submit'])){
                    $payer_acc = trim($_POST['payer_acc']);
                    $payee_acc = trim($_POST['payee_acc']);
                    $amount = trim($_POST['amount']);
                    $description = trim($_POST['description']);
                    if($payer_acc == $payee_acc){
                        echo "Account Number are the same";
                    }else{
                    $sel = "SELECT Acc_No, Acc_Type, Balance FROM Account_tbl where Acc_No ='$payer_acc'";
                    ...
                    $select = "SELECT Acc_No, Acc_Type, Balance FROM Account_tbl where Acc_No ='$payee_acc'";
                    $query=mysqli_query($conn,$select);
                    $row_num = mysqli_num_rows($query);
                    while ($row=mysqli_fetch_assoc($query)){
                        $bal = $row ['Balance'];
                    ...
                    if ($row_num == 0){
                        echo "You cannot tranfer to the same account or the number is invalid";
                        }else{
                        $total = $balance - $amount;
                        $total2 = $bal + $amount;
                        if($total >= 0){
                        $insert = "INSERT INTO Account_tbl (Acc_No,Acc_Type,Withdrawal,Description,Balance)
                        VALUES('$Acc_No','$Acc_Type','$amount','$description','$total')";
                        $result=mysqli_query($conn,$insert);

                            
                        $int = "INSERT INTO Account_tbl (Acc_No,Acc_Type,Deposit,Description,Balance)
                        VALUES('$Acc_Num','$Type','$amount','$description','$total2')";
                        $result1=mysqli_query($conn,$int);
                        echo "<strong>You have successfully Transfer the Sum of: {$amount} From: {$payer_acc} To: {$payee_acc}</strong>";
                        echo "<h3>Thanks for Banking With Us</h3>";
                        }else{
            ...
            <form method="post" enctype="multipart/form-data" class="p-4  border rounded-3 bg-light">
                <h3>FUND TRANSFER</h3>
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>PAYER ACCOUNT NO:</strong></span>
                    <input type="number" class="form-control"  required name="payer_acc" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>PAYEE ACCOUNT NO:</strong></span>
                    <input type="number" class="form-control"  required name="payee_acc" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>AMOUNT:</strong></span>
                    <input type="number" class="form-control"  required name="amount" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default">
                </div>
                <div class="input-group mb-3">
                    <span class="input-group-text" id="inputGroup-sizing-default"><strong>Description:</strong></span>
                    <textarea class="form-control" row="5" required name="description" aria-label="With textarea"></textarea>
                </div>
                <div class="input-group mb-3">
