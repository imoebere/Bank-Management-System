      ...
      <div class="col-md-10 col-lg-6">
          <form method="post" enctype="multipart/form-data" class="p-4  border rounded-3 bg-light">
                <div class="input-group mb-3">
                    ...
                    <select class="form-select" name="acc" required id="inputGroupSelect01">
                ...
                <?php
                    if(isset($_POST['Submit'])){
                        $acc = $_POST['acc'];
                        $select = "SELECT Acc_No, Acc_Type, Balance FROM Account_tbl where Acc_No ='$acc'";
                        $query=mysqli_query($conn,$select);
                        $row_num = mysqli_num_rows($query);
                        while ($row=mysqli_fetch_assoc($query)){
                                $Acc_Num= $row ['Acc_No'];
                        ...
                        echo "<strong>Your Account Number: {$Acc_Num}<br/>";
                        echo "<strong>Your Account Balence: {$balance}<br/>";
                        echo "<strong>Your Account Type: {$Type}<br/>";
                    }
